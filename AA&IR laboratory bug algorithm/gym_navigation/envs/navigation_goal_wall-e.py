""" This module contains the Navigation Goal environment class with Disney Wall-e """
...
class NavigationGoalDisney(Navigation):
    """ The Navigation Goal environment """

    GOAL_THRESHOLD = 1  # goal radius
    COLLISION_THRESHOLD = 1.5  # robot radius
    OBSTACLE_RADIUS = 1.5
    MINIMUM_DISTANCE_ROBOT_GOAL = 20  # min distance between robot and goal
    MINIMUM_DISTANCE_OBSTACLE = 5  # min distance between obstacles
    MINIMUM_DISTANCE = 5  # min distance among obstacles, robot and goal
    MINIMUM_SAFE_DISTANCE = 3.3
    N_OBSTACLES = 3
    ...
    def __init__(self, render_mode=None, track_id: int = 1) -> None:
        ...
        pygame.display.set_caption('Wall-e Navigation Goal')
    ...
    def draw(self, canvas: Surface) -> None:
        ...
        # CIRCULAR OBSTACLES
        icon_obstacle = pygame.image.load('./gym_navigation/images/rock.png')
        for obstacle in self.circular_obstacles:
            # pygame.draw.circle(canvas,
            #                    Color.BLUE.value,
            #                    self.convert_point(obstacle),
            #                    self.OBSTACLE_RADIUS * self.RESOLUTION)
            canvas.blit(icon_obstacle, self.rock_convert_point(obstacle))
        ...
        icon_goal = pygame.image.load('./gym_navigation/images/plant.png')
        canvas.blit(icon_goal, self.plant_convert_point(self.goal))
        ...
        icon_robot = pygame.image.load('./gym_navigation/images/wall-e.png')
        canvas.blit(icon_robot, self.walle_convert_point(self.pose.position))
        # image printed with upper left corner in position by default
    ...
    def rock_convert_point(self, point: Point) -> Tuple[int,int]:
        shiftX = 33
        shiftY = 24
    ...
    def walle_convert_point(self, point: Point) -> Tuple[int,int]:
        shiftX = 23
        shiftY = 29
        pygame_x = (round(point.x_coordinate * self.RESOLUTION) - shiftX + self.X_OFFSET)
        pygame_y = (self.window_size - round(point.y_coordinate * self.RESOLUTION) - shiftY + self.Y_OFFSET)
    ...
    def plant_convert_point(self, point: Point) -> Tuple[int,int]:
        shiftX = 33
        shiftY = 40
        pygame_x = (round(point.x_coordinate * self.RESOLUTION) - shiftX + self.X_OFFSET)
        pygame_y = (self.window_size - round(point.y_coordinate * self.RESOLUTION) - shiftY + self.Y_OFFSET)
